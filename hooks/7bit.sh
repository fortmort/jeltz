...
# interprets that as "please add more."  We check for the presence of
# non-7-bit ASCII characters and instruct Claude Code what to do
...
#
# Enforcement scope is repo-aware (see lib/repo-mode.sh). In a tree that ships
# no standard of its own, only lines changed against HEAD are policed. This
# costs the hook nothing as a slop guard: emoji, check marks and box-drawing
# banners are by construction newly written, so they are always in the diff.
# What it does buy is silence on the pre-existing content the rest of the repo
# is already full of -- a guard that cries wolf on most of the tree is a guard
...
get_codepoint() {
    ...
    hex_val=$(printf '%s' "$char" | iconv -f UTF-8 -t UTF-32BE 2>/dev/null | od -An -tx1 | tr -d ' \n')
    if [[ -n "$hex_val" ]]; then
...
if [[ -f "$FILE_PATH" && -r "$FILE_PATH" ]]; then
    ...
    if [[ "$MIME_TYPE" == text/* ||
        ...
        "$MIME_TYPE" == inode/x-empty ]]; then # empty text files

        # lines containing non-7-bit ASCII
        OFFENDING_LINES=$("${GREP_CMD}" -nP '[^\x00-\x7F]' "$FILE_PATH")

        # Restrict to lines we actually wrote, unless this tree declares its own
        # standard (strict) or the file is new/untracked/outside git ("ALL"),
        # in which case every line is fair game.
        HOOK_MODE=$(hook_repo_mode "$FILE_PATH")
        CHANGED_LINES=$(hook_changed_lines "$FILE_PATH")
        if [[ "$HOOK_MODE" != "strict" && "$CHANGED_LINES" != "ALL" ]]; then
            if [[ -z "$CHANGED_LINES" ]]; then
                OFFENDING_LINES=""
            else
                # Passed through the environment rather than -v: awk's -v
                # mangles embedded newlines, and this value is a line list.
                OFFENDING_LINES=$(echo "$OFFENDING_LINES" |
                    CHANGED_LINES="$CHANGED_LINES" awk '
                    BEGIN {
                        n = split(ENVIRON["CHANGED_LINES"], a, "\n")
                        for (i = 1; i <= n; i++) keep[a[i]] = 1
                    }
                    { idx = index($0, ":"); if (substr($0, 1, idx - 1) in keep) print }')
            fi
        fi
        ...
        if [[ -n "$OFFENDING_LINES" ]]; then
            ...
            # Count from the (possibly diff-scoped) offending lines, not the
            # whole file, so the total matches what is actually reported.
            FILE_NON_ASCII_COUNT=$(echo "$OFFENDING_LINES" | "${GREP_CMD}" -oP '[^\x00-\x7F]' | wc -l)
