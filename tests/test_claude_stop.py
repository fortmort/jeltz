"""Tests for T16: the Claude Code Stop hook shim.

The shim is the thin host-specific layer over T15's bridge: it reads the
Stop-hook payload from stdin, asks `attempt_stop` for a ruling, and
translates a denial into Claude Code's documented Stop decision schema -
a top-level `{"decision": "block", "reason": ...}` on stdout - carrying
the recovery instruction as the reason. (`hookSpecificOutput` decisions
belong to other events such as PreToolUse; the Stop schema is top-level.)
Allows produce no output at all: silence plus exit 0 is Claude Code's
"proceed".

Host-specific duties pinned here:
- `stop_hook_active` is respected: when Claude Code reports it is already
  continuing because of a Stop hook, the shim allows immediately and
  consults nothing - it must never contribute to a stop-hook loop, and it
  must not record a denial for a stop it never gated.
- Never block twice for the same tree: the bridge's repeat-denial guard
  reaches the host through the shim, so a second stop attempt for an
  already-denied hash is allowed.
- The shim inherits the fail-open posture: input it cannot parse or a
  payload without a usable `cwd` allows the stop with a logged error,
  because a crashing or blocking hook on bad input would trap the session
  (CI is the backstop, R4).
"""
...
from review.claude_stop import main
...
def stop_payload(repo: Path, *, active: bool = False) -> str:
    """A Claude Code Stop-hook input payload for the given repo."""
    return json.dumps(
        {
...
def test_denied_session_reaches_acceptance_and_stops(
    ...
    """T16 acceptance: the T15 loop works end to end through the shim.

    A session edits tracked source and tries to stop: the shim denies once
    with the instruction, the session runs the literal command against a
    scripted backend, reaches acceptance, and the next stop is silent.
    """
    (repo / "src.py").write_text("VALUE = 2\n")
    assert run_hook(monkeypatch, stop_payload(repo)) == 0
