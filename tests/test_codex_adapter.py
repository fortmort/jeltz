...
def test_fresh_review_runs_exec_with_schema_and_json(dirty_repo: Path, tmp_path: Path) -> None:
...
def test_resume_runs_exec_resume_on_the_prior_thread(dirty_repo: Path, tmp_path: Path) -> None:
...
def test_bare_schema_json_is_normalized_for_the_parser(dirty_repo: Path, tmp_path: Path) -> None:
...
def test_fenced_output_is_passed_through_unwrapped(dirty_repo: Path, tmp_path: Path) -> None:
...
def test_the_last_agent_message_carries_the_verdict(dirty_repo: Path, tmp_path: Path) -> None:
...
def test_garbage_lines_in_the_stream_are_ignored(dirty_repo: Path, tmp_path: Path) -> None:
...
def test_json_lines_that_are_not_objects_are_ignored(dirty_repo: Path, tmp_path: Path) -> None:
...
def test_non_verdict_json_message_is_not_fenced(dirty_repo: Path, tmp_path: Path) -> None:
...
def test_backend_nonzero_exit_is_a_typed_error(dirty_repo: Path, tmp_path: Path) -> None:
    ...
    home = install_fake_codex(tmp_path, [{"stdout": "", "stderr": "codex: boom", "exit": 1}])
...
def test_hung_backend_is_killed_into_a_typed_error(dirty_repo: Path, tmp_path: Path) -> None:
    ...
    home = install_fake_codex(tmp_path, [{"sleep": 30, "stdout": events(bare_verdict())}])
...
def test_backend_cannot_wait_on_the_callers_stdin(dirty_repo: Path, tmp_path: Path) -> None:
    ...
    home = install_fake_codex(tmp_path, [{"read_stdin": True, "stdout": events(bare_verdict())}])
...
def test_stream_without_thread_id_is_a_continuity_error(dirty_repo: Path, tmp_path: Path) -> None:
...
def test_stream_without_agent_message_is_empty_output(dirty_repo: Path, tmp_path: Path) -> None:
...
def test_repair_round_resumes_the_review_thread(dirty_repo: Path, tmp_path: Path) -> None:
